...
app.post("/api/register", async (c) => {
  ...
  try {
    await c.env.DB.prepare("INSERT INTO users (name) VALUES (?)").bind(registername).run();
...
app.post("/api/search", async (c) => {
  ...
  try {
    const result = await c.env.DB.prepare("SELECT * FROM users WHERE name = ?").bind(searchname).first();
    if (result) {
...
app.get("/api/tasks", async (c) => {
  const username = await c.req.query("name");
  try {
...
app.put("/api/edit/:id", async (c) => {
  const id = c.req.param("id");
  const param = await c.req.json();
  const completed = param.completed ? 1 : 0;
  console.log(param);
  try {
    await c.env.DB.prepare("UPDATE tasks SET completed = ? WHERE id = ?").bind(completed, id).run();
...
// ../../.nvm/versions/node/v23.3.0/lib/node_modules/wrangler/templates/middleware/middleware-ensure-req-body-drained.ts
...
// ../../.nvm/versions/node/v23.3.0/lib/node_modules/wrangler/templates/middleware/middleware-miniflare3-json-error.ts
...
// .wrangler/tmp/bundle-RbygF3/middleware-insertion-facade.js
...
// ../../.nvm/versions/node/v23.3.0/lib/node_modules/wrangler/templates/middleware/common.ts
...
// .wrangler/tmp/bundle-RbygF3/middleware-loader.entry.ts
